...
VALUES ('*/20 * * * *', $$
    SELECT aave.insert_aave_interest(
        (SELECT MAX(evt_block_time) - interval '1 days' FROM aave.aave_interest),
        (SELECT now() - interval '20 minutes'),
        (SELECT MAX(number) FROM ethereum.blocks WHERE time < (SELECT MAX(evt_block_time) - interval '1 days' FROM aave.aave_interest)),
